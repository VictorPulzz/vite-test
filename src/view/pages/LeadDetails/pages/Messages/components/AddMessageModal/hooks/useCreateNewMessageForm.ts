...
interface UseAddNewMessageFormProps {
  onSubmitSuccessful?: () => void;
}
export function useCreateNewMessageForm({
  onSubmitSuccessful,
}: UseAddNewMessageFormProps): UseCreateNewMessageFormReturn {
  const params = useParams();
  const leadId = useMemo(() => String(params.id), [params.id]);
  ...
  const handleSubmit = useCallback(
    ...
    [createMessage, leadId, onSubmitSuccessful],
  );
